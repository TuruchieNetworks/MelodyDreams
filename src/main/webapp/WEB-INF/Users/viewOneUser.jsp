   	...
   	onmouseout="changePlayerCover(this)" >
	    <ul class="navbar-list" style=""> 
	        ...
	        <li class="panel-card point-border-radius sm-padding" style="">
			    <form class="playerCover imageCover point-border-radius" action="/melodydreams/artists/${oneUser.id}"style="">    
			   	...
			   	style="background-repeat: no-repeat;background-position: center;background-size: cover;position:fixe;">
	                <li class="navShowcase playerCover imageCover"
					    onmouseover="this.style.backgroundImage='url(${pageContext.request.contextPath}/img/purplecrowd-unsplash.jpg)'"
					 	onmouseout="changePlayerCover(this)" 
					   	style=";background-repeat: no-repeat;background-position: center;background-size: cover;">
					   	<a class="bluebtn panel-card mid-fonts word-slicer party-lights" href="/melodydreams/artists/${loggedInUser.id}" style="background:rgba(0.1, 0.2, 0.9, 0.7);">
	     ...
	     <div class="playerCover flexCover lrg-padding" style="">
				...
				onmouseover="this.style.transition = 'background-image 1.2s ease-in-out'; this.style.backgroundImage='url(${pageContext.request.contextPath}/img/landing-img.jpg)'"   
				  ...
				  <div class="profileShowcase playerCover columnCover display-block over-flow playlistContainer" id="" style="" >
					  <c:forEach items="${allUsersTrackList}" var="song">
					  ...
					  style=" background-image:url( ${pageContext.request.contextPath}/FileUploads/${song.trackImageDataUrl}/${song.trackImageFileName});">	    			
							...
							<input type="hidden" id="musicListData2" value="${userMusicList}"/>
										    ...
										    <div id="${song.id}" class="flex-panel panel-card full-width" style=""  onclick="panelPlaypauseTrack(this)">
										        <div class="flex-panel sm-hor-margin outer-panel-player-cover " style="">
											        ...
											        <span id="${song.id}" class="spreader bright-flash upper-panel-play-btn">
											       	...
											       	<c:choose>
													    <c:when test="${song.likedSongs.size() == 0}">
								        					<span>
								        						<c:out value="No Likes"/>
								        					</span>
								        				</c:when>
													    <c:when test="${song.likedSongs.size() == 1}">
								        					<span>
								        						<c:out value="Liked By ${song.likedSongs.size()} User"/>
								        					</span>
													    </c:when>
													    <c:otherwise>
								        					<span>
								        						<c:out value="Liked By ${song.likedSongs.size()} Users"/>
								        					</span>
								        				</c:otherwise>
								        			</c:choose>
										 	...
										 	<div id="non-id-play-panel-${song.id}" class="lead flexCover non-id-play-panel bright-colors hidePanel" style="display:none" onclick="playpauseTrack(this)">
											    <div class="flex-panel panel-car playerCover " style="">
											        <div id="${song.id}" class="flex-panel sm-hor-margin pause-panel" style="">
												        <c:out value="Play ${song.trackTitle}"/>
												        <span id="${song.id}" class="spreader ">
												        </span> <!-- Span to hold the play/pause icon -->
												        <span id="${song.id}" class="bright-colors upper-panel-play-btn playerCover">
								        					<i id="${song.id}"  class="playerCover upper-track-art-widget fa fa-play-circle fa-5x">
								        					</i>
													    </span> 
													    <span>
													       	<i class="sm-fonts type-writer word-slicer">
							        	...
							        	<div class="main-media-panel hidePanel" id="${song.id}" style="font-size:12px;">
									        ...
									        onmouseout="this.style.transition = 'background-image 1s ease-in-out'; this.style.backgroundImage='url(${pageContext.request.contextPath}/FileUploads/${song.trackImageDataUrl}/${song.trackImageFileName}'"
			      							onmouseover="this.style.transition = 'background-image 1s ease-in'; this.style.backgroundImage='url(${pageContext.request.contextPath}/img/landing-img.jpg)'">
										        ...
										        <div id="upper-panel-play-pause-icon-${song.id}" class="playpause-track" onclick="playpauseTrack(this)">
										             <i class=""></i>
										        </div>
											...
											<c:if test="${loggedInUser.id == song.user.id}">	
												...
												<!-- Prompts to open or close play-list forms -->
												<p class="show-new-playlist-prompt leadShowcase sm-vert-margin" onclick="showNewPlaylistForm()">
													<c:out value="Create New Playlist"/>
	        										<i class="fa fa-chevron-down new-playlist-chevron-icon"></i>
												</p>  
	
												<p class="show-playlist-prompt leadShowcase sm-vert-margin" onclick="showPlaylistForm()">
													<c:out value="Add Song To Playlist"/>
	        										<i class="fa fa-chevron-down playlist-chevron-icon"></i>
												</p>
	
												<!-- Form to create a new playlist and add a new song to it -->
				        						 <c:if test="${song.user.playlists.size() > 1}">
													<form id="newPlaylistForm-${song.id}" class="new-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylist/${song.id}" method="POST" style="display:none" modelAttribute="playlist">
				        						 ...
				        						 <c:choose>
											        ...
											        <c:otherwise>  
													<!-- Form to add a new song to existing playlist-->
												        <c:forEach items="${loggedInUser.playlistSongs}" var="track">
															<form id="playlistForm-${song.id}" class="playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylistSong/${song.id}" method="POST" style="" modelAttribute="playlistSong">
													            <select name="song_id" class="panel-card lead track-creation-input full-width round-border-radius sm-vert-margin" style="cursor:pointer">
												           		...
												           		<input type="hidden" name="song_id" value="${song.id}">
												           		<input type="hidden" name="user_id" value="${loggedInUser.id}">
														   		<input class="leadShowcase" type="submit" value="Add Song To ${track.playlist.title} Playlist">
														    </form>
													    </c:forEach>
												     </c:otherwise>
												 </c:choose>
												 ...
												 <div id="mainSocialActionsPanel-${song.id}" class="social-actions-panel leadShowcase full-width">
												     <!-- Like Song Form -->
												     <div id="trackStatId-${song.id}" class="track-stat leadShowcase full-width hidePanel">
												          <div class="">
							        						  <c:choose>
														        <c:when test="${song.likedSongs.size() < 1}">	
															        <form id="likeSongForm-${song.id}" class="like-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/process/createNewLikedSong/${song.id}" method="POST">
															            <input type="hidden" name="likingUserId" value="${loggedInUser.id}">
															            <input type="hidden" name="user_id" value="${loggedInUser.id}">
														        ...
														        <c:otherwise>
																	...
																	<c:forEach items="${song.likedSongs}" var="likedSong">
																	    <c:if test="${loggedInUser.id == likedSong.user.id}">
																	        <form id="unlikeSongForm-${likedSong.id}" class="unlike-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/deleteLikedSong/${likedSong.id}" method="POST">
																	            ...
																	            <button class="leadShowcase input-slicer" type="submit">
																	            	<c:out value="Unlike ${likedSong.song.trackTitle}"/>
																	            </button>
																	        </form>
																	    </c:if>
																	</c:forEach>
																...
																<p  id="show-favorite-panel-paragraph-${song.id}" class="show-favorite-panel-paragraphs leadShowcase sm-vert-margin" onclick="showFavoriteSongForm()">
																    <c:out value="Show Favorites Panel"/>
																    <i id="upper-favorites-chevron-icon-${song.id}" class="fa fa-chevron-down upper-favorites-chevron-icon"></i>
																</p>			   		         
	
																<div id ="favorite-song-panel-${song.id}" class="flex-btw upper-favorite-song-panel hidePanel" style="display:none">
																	<c:choose>
																        <c:when test="${song.favoriteSongs.size() < 1}">	
																	        <form id="favoriteSongForm-${song.id}" class="favorite-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/process/createNewFavoriteSong/${song.id}" method="POST">
																	            <input type="hidden" name="song_id" value="${song.id}">
																	            <input type="hidden" name="user_id" value="${loggedInUser.id}">
																	            <input class="leadShowcase input-slicer" type="submit" value="Add ${song.trackTitle} To Favorites">
																	        </form>
																         </c:when>
																         <c:otherwise>
																	        <!-- Unfavorite Song Form -->
																			<c:forEach items="${song.favoriteSongs}" var="favoriteSong">
																			    <c:if test="${loggedInUser.id == favoriteSong.user.id}">
																			        <form id="unfavoriteSongForm-${favoriteSong.id}" class="delete-favorite-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/deleteFavoriteSong/${favoriteSong.id}" method="POST">
																			            <input type="hidden" name="song_id" value="${favoriteSong.song.id}">
																			            <input type="hidden" name="user_id" value="${loggedInUser.id}">
																			            <input type="hidden" name="_method" value="DELETE">
																			            <button class="leadShowcase input-slicer" type="submit">
																			            	<c:out value="Remove ${favoriteSong.song.trackTitle} From Favorites"/>
																			            </button>
																			        </form>
																			      </c:if>
																			   </c:forEach>
															...
															<!-- Artist Social Panel -->
															<p class="show-follow-user leadShowcase sm-vert-margin" onclick="showFollowForm()">
															    <c:out value="${song.trackArtist} Social Panel "/>
															    <i class="fa fa-chevron-down follow-user-chevron-icon"></i>
															</p>
															
														    <div id ="social-actions-Panel-${song.id}" class="flex-btw follow-user-panel hidePanel" style="display:none;">
														        <!-- Like User Form -->
														        <div class="flex-btw">
									        						 <c:choose>
																        <c:when test="${song.user.likedUsers.size() < 1}">	
																        ...
																        <c:forEach items="${song.user.likedUsers}" var="likedUser">
																		    <c:if test="${loggedInUser.id == likedUser.user.id}">
																		        <form id="unlikeUserForm-${likedUser.id}" class="unlike-user-form playerCover leadShowcase mid-margin full-width" action="/melodydreams/deleteLikedUser/${likedUser.id}" method="POST">
																		            <input type="hidden" name="likingUserId" value="${loggedInUser.id}">
																		            <input type="hidden" name="user_id" value="${loggedInUser.id}">
																		            <input type="hidden" name="_method" value="DELETE">
															    ...
															    <!-- Follow User Form -->
															    <form id="followUserForm-${song.id}" class="follow-user-form playerCover leadShowcase mid-vert-margin full-width hidePanel" action="/melodydreams/process/createNewFollowedUser/${song.user.id}" method="POST">
													...
													<p class="show-delete delete-prompt leadShowcase sm-vert-margin" onclick="showDeleteForm()">
											 		...
											 		<form id="delete-${song.id}" class="deleteForm upper-delete-form leadShowcase mid-vert-margin hidePanel" action="/melodydreams/deleteTrack/${song.id}" method="get" style="background:rgba(1.33, 10.64, 0.60, 0.9);">
													    <input type="hidden" name="_method" value="DELETE">
							...
							<div id=${song.id } class="section-div-slicer playerCover imageCover x-lrg-margin-bottom mid-padding-bottom round-border-radius" onmouseout="changePlayerCover(this)">	
								...
								<div class="leadShowcase round-border-radius imageCover" style="
									...
									<div class ="section-media-panel hidePanel" id="${song.id}">
										...
										<!-- Prompts to open or close play-list forms -->
										<p class="show-section-new-playlist-prompt leadShowcase sm-vert-margin" onclick="showNewPlaylistForm()">
											<c:out value="Create New Playlist"/>
       										<i class="fa fa-chevron-down new-section-playlist-chevron-icon"></i>
										</p>  
	
										<p class="show-section-playlist-prompt leadShowcase sm-vert-margin" onclick="showPlaylistForm()">
											<c:out value="Add Song To Playlist"/>
										...
										<!-- Form to create a new playlist and add a new song to it -->
		        						 <c:if test="${song.user.playlists.size() > 1}">
											<form id="newSectionPlaylistForm-${song.id}" class="new-section-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylist/${song.id}" method="POST" style="display:none" modelAttribute="playlist">
										        <input class="playlist-input leadShowcase full-width sm-vert-margin" name="title" placeholder="Title Of Your Play List">
										        <textarea class="leadShowcase full-width sm-vert-margin" name="description" placeholder="Description"></textarea>
								           		<input type="hidden" name="user_id" value="${loggedInUser.id}">
										   		<input class="leadShowcase" type="submit" value="Create New Playlist" onclick="handlePlaylisForm()">
		        						 ...
		        						 <c:choose>
									        <c:when test="${loggedInUser.playlists.size() < 1}">	
												<form id="sectionPlaylistForm-${song.id}" class="section-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylist/${song.id}" method="POST" style="" modelAttribute="playlist">
										            <input class="playlist-input leadShowcase full-width sm-vert-margin" name="title" placeholder="Title Of Your Play List">
										            <textarea class="leadShowcase full-width sm-vert-margin" name="description" placeholder="Description"></textarea>
									           		<input type="hidden" name="user_id" value="${loggedInUser.id}">
											   		<input class="leadShowcase" type="submit" value="Create New Playlist" onclick="handlePlaylisForm()">
											...
											<!-- Form to add a new song to existing playlist-->
										        <c:forEach items="${loggedInUser.playlistSongs}" var="track">
													<form id="sectionPlaylistForm-${song.id}" class="section-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylistSong/${song.id}" method="POST" style="" modelAttribute="playlistSong">
											            <select name="song_id" class="panel-card lead track-creation-input full-width round-border-radius sm-vert-margin" style="cursor:pointer">
											                 <option value="" class="leadShowcase text-primary sm-vert-margin mid-padding">Select Playlist</option>
											                 <option value="${track.playlist.id}" class="leadShowcase text-primary sm-vert-margin mid-padding" style="cursor:pointer;">${track.playlist.title}</option>												               
											            </select>
										 ...
										 <div id="socialActionsPanel-${song.id}" class="social-actions-panel leadShowcase full-width">
										    ...
										    <div id="trackStatId-${song.id}" class="section-track-stat leadShowcase full-width hidePanel">
											   <div class="">
													<c:if test="${loggedInUser.likedUsers.size() < 1}">
												        <form id="likeSongForm-${song.id}" class="like-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/process/createNewLikedSong/${song.id}" method="POST">
												            <input type="hidden" name="song_id" value="${song.id}">
												            <input type="hidden" name="user_id" value="${loggedInUser.id}">
												            <!-- i class="fa fa-solid fa-heart"></i-->
													...
													<c:forEach items="${song.likedSongs}" var="likedSong">
													    <c:if test="${loggedInUser.id == likedSong.likingUserId}">
													        <form id="unlikeSongForm-${likedSong.id}" class="unlike-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/deleteLikedSong/${likedSong.id}" method="POST">
													            ...
													            <input type="hidden" name="user_id" value="${loggedInUser.id}">
													            <input type="hidden" name="_method" value="DELETE">
													            <button class="bright-flash leadShowcase input-slicer" type="submit">
													            	<c:out value="Unlike ${likedSong.song.trackTitle}"/>
													            </button>
													        </form>
													    </c:if>
	 	          								...
	 	          								<!-- Section Favorites Actions Button -->
												<p id="show-section-favorite-panel-paragraph-${song.id}" class="show-section-favorite-panel-paragraphs leadShowcase sm-vert-margin" onclick="showFavoriteSongForm()">
													<c:out value="Show Favorites Panel"/>
												    <i id="section-favorites-chevron-icon-${song.id}" class="fa fa-chevron-down section-favorites-chevron-icon"></i>
												</p>			   		         
												<div id ="section-favorite-song-panel-${song.id}" class="flex-btw section-favorite-song-panel hidePanel" style="display:none">
													<c:choose>
												        <c:when test="${song.favoriteSongs.size() < 1}">	
													        <form id="favoriteSongForm-${song.id}" class="favorite-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/process/createNewFavoriteSong/${song.id}" method="POST">
													            <input type="hidden" name="song_id" value="${song.id}">
												...
												<!-- Section Artist Social Panel -->
												<p class="show-section-follow-user leadShowcase sm-vert-margin" onclick="showFollowForm()">
												    <c:out value="${song.trackArtist} Social Panel "/>
												    <i class="fa fa-chevron-down section-follow-user-chevron-icon"></i>
												</p>
														
												<div id ="section-social-actions-Panel-${song.id}" class="flex-btw section-follow-user-panel hidePanel" style="display:none">
											        <!-- Like User Form -->
											        <div class="flex-btw">
														<c:if test="${loggedInUser.likedUsers.size() < 1}">
													        <form id="likeUserForm-${song.id}" class="like-user-form playerCover leadShowcase mid-margin full-width" action="/melodydreams/process/createNewLikedUser/${song.user.id}" method="POST">
												        ...
												        <c:forEach items="${song.user.likedUsers}" var="likedUser">
														    <c:if test="${loggedInUser.id == likedUser.user.id}">
														        <form id="unlikeUserForm-${likedUser.id}" class="unlike-user-form playerCover leadShowcase mid-margin full-width" action="/melodydreams/deleteFollowedUser/${followedUser.id}" method="POST">
														            ...
														            <input type="hidden" name="_method" value="DELETE">
														            <button class="leadShowcase input-slicer" type="submit">
														            	<c:out value="Unlike ${likedUser.user.firstName}"/>
														            </button>
														        </form>
														    </c:if>
														 </c:forEach>
											      	...
											      	<c:forEach items="${song.user.followedUsers}" var="followedUser">
													    <c:if test="${loggedInUser.id == followedUser.user.id}">
													        <form id="unfollowUserForm-${followedUser.id}" class="unfollow-user-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/deleteFollowedUser/${followedUser.id}" method="POST">
													            ...
													            <input type="hidden" name="user_id" value="${loggedInUser.id}">
													            <input type="hidden" name="_method" value="DELETE">
													            <button class="bright-colors leadShowcase input-slicer" type="submit">
													            	<c:out value="Unfollow ${followedUser.user.firstName}"/>
													            </button>
													        </form>
													    </c:if>
													</c:forEach>
											...
											<p class="show-section-delete delete-prompt leadShowcase sm-vert-margin" onclick="showDeleteForm()">
									 		...
									 		<form id="section-delete-${song.id}" class="deleteForm section-delete-form leadShowcase mid-vert-margin hidePanel" action="/melodydreams/deleteTrack/${song.id}" method="get" style="background:rgba(1.33, 10.64, 0.60, 0.9);">
											    <input type="hidden" name="_method" value="DELETE">
											    <input class ="delete-input  point-border-radius lrg-padding panel-card" id="${song.id}" type="submit" value="Delete Your ${song.trackTitle} Track" style="width: 100%;padding:7px;width:100%;" >
											</form> 
											</c:if>
						      	...
						      	<div  id="id-play-panel-${song.id}" class="lead flexCover bright-colors track-panel id-play-panel" style="">
								    <div id="${song.id}" class="flex-panel panel-card full-width" style=""  onclick="panelPlaypauseTrack(this)">
								        <div class="flex-panel sm-hor-margin footer-outer-panel-player-cover" style="">
									        ...
									        <span id="span-id-play-panel-${song.id}" class="playpause-icon">
									        </span> <!-- Span to hold the play/pause icon -->
									        <span id="${song.id}" class="spreader bright-flash upper-panel-play-btn">
					        ...
					        <div class="footer-media-panel hidePanel" id="${song.id}" style="">
								...
								<c:if test="${loggedInUser.id == song.user.id}">	
									<div class="leadShowcase mid-padding">
										...
										<!-- Prompts to open or close play-list forms -->
										<p class="show-footer-new-playlist-prompt leadShowcase sm-vert-margin" onclick="showNewPlaylistForm()">
											<c:out value="Create New Playlist"/>
       										<i class="fa fa-chevron-down new-footer-playlist-chevron-icon"></i>
										</p>  
	
										<p class="show-footer-playlist-prompt leadShowcase sm-vert-margin" onclick="showPlaylistForm()">
											<c:out value="Add Song To Playlist"/>
										...
										<!-- Form to create a new playlist and add a new song to it -->
		        						 <c:if test="${song.user.playlists.size() > 1}">
											<form id="newFooterPlaylistForm-${song.id}" class="new-footer-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylist/${song.id}" method="POST" style="display:none" modelAttribute="playlist">
										        <input class="playlist-input leadShowcase full-width sm-vert-margin" name="title" placeholder="Title Of Your Play List">
										        <textarea class="leadShowcase full-width sm-vert-margin" name="description" placeholder="Description"></textarea>
								           		<input type="hidden" name="user_id" value="${loggedInUser.id}">
										   		<input class="leadShowcase" type="submit" value="Create New Playlist" onclick="handlePlaylisForm()">
		        						 ...
		        						 <c:choose>
									        <c:when test="${loggedInUser.playlists.size() < 1}">	
												<form id="footerPlaylistForm-${song.id}" class="footer-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylist/${song.id}" method="POST" style="" modelAttribute="playlist">
										            <input class="playlist-input leadShowcase full-width sm-vert-margin" name="title" placeholder="Title Of Your Play List">
										            <textarea class="leadShowcase full-width sm-vert-margin" name="description" placeholder="Description"></textarea>
									           		<input type="hidden" name="user_id" value="${loggedInUser.id}">
											   		<input class="leadShowcase" type="submit" value="Create New Playlist" onclick="handlePlaylisForm()">
											...
											<!-- Form to add a new song to existing playlist-->
									        <c:forEach items="${loggedInUser.playlistSongs}" var="track">
												<form id="footerPlaylistForm-${song.id}" class="footer-playlist-form playerCover hidePanel" action="/melodydreams/process/createNewPlaylistSong/${song.id}" method="POST" style="" modelAttribute="playlistSong">
										            <select name="song_id" class="panel-card lead track-creation-input full-width round-border-radius sm-vert-margin" style="cursor:pointer">
										                 <option value="" class="leadShowcase text-primary sm-vert-margin mid-padding">Select Playlist</option>
										                 <option value="${track.playlist.id}" class="leadShowcase text-primary sm-vert-margin mid-padding" style="cursor:pointer;">${track.playlist.title}</option>												               
										            </select>
										...
										<div id="socialActionsPanel-${song.id}" class="social-actions-panel leadShowcase bluebtn">
											...
											<!-- Section Favorites Actions Button -->
											<p id="show-footer-favorite-panel-paragraph-${song.id}" class="show-footer-favorite-panel-paragraphs leadShowcase sm-vert-margin" onclick="showFavoriteSongForm()">
												<c:out value="Show Favorites Panel"/>
											    <i id="footer-favorites-chevron-icon-${song.id}" class="fa fa-chevron-down footer-favorites-chevron-icon"></i>
											</p>			   		         
	
											<div id ="footer-favorite-song-panel-${song.id}" class="flex-btw footer-favorite-song-panel hidePanel" style="display:none">
												<c:choose>
											        <c:when test="${song.favoriteSongs.size() < 1}">	
												        <form id="favoriteSongForm-${song.id}" class="favorite-song-form playerCover leadShowcase mid-vert-margin full-width" action="/melodydreams/process/createNewFavoriteSong/${song.id}" method="POST">
												            <input type="hidden" name="song_id" value="${song.id}">
											...
											<!--Footer Artist Social Panel -->
											<p class="show-footer-follow-user leadShowcase sm-vert-margin" onclick="showFollowForm()">
											    <c:out value="${song.trackArtist} Social Panel "/>
											    <i class="fa fa-chevron-down footer-follow-user-chevron-icon"></i>
											</p>
																
											<div id ="footer-social-actions-Panel-${song.id}" class="flex-btw footer-follow-user-panel hidePanel" style="display:none">
												<!-- Like User Form -->
										        <div class="flex-btw">
									        	<c:choose>
													<c:when test="${song.user.likedUsers.size() < 1}">	
											        <form id="likeUserForm-${song.user.id}" class="like-user-form playerCover leadShowcase mid-margin full-width" action="/melodydreams/process/createNewLikedUser/${song.user.id}" method="POST">
											            <input type="hidden" name="user_id" value="${loggedInUser.id}">
											            <input class="leadShowcase input-slicer" type="submit" value="Like ${song.trackArtist}">
											        </form>
											        </c:when>
										...
										<p class="show-footer-delete delete-prompt leadShowcase sm-vert-margin" onclick="showDeleteForm()">
								 		...
								 		<form id="footer-delete-${song.id}" class="deleteForm footer-delete-form leadShowcase mid-vert-margin hidePanel" action="/melodydreams/deleteTrack/${song.id}" method="get" style="background:rgba(1.33, 10.64, 0.60, 0.9);">
										    <input type="hidden" name="_method" value="DELETE">
										    <input class ="delete-input  point-border-radius lrg-padding panel-card" id="${song.id}" type="submit" value="Delete Your ${song.trackTitle} Track" style="width: 100%;padding:7px;width:100%;" >
										</form> 
									</div>
...
<p id="playCounts" data-play-count-list="" class="hidePanel">
	${playCounts}
</p>
	
<p id="musicListData" data-musiclist="" class="hidePanel">
	${userMusicList}
</p>
	
<p id="loggedUser" data-musiclist="" class="hidePanel">
	${loggedInUser.id}
</p>
<link rel="stylesheet" href="/styles/main_player.css"/>
<link rel="stylesheet" href="/styles/privateStyles.css"/>
<script>
    ...
    const deleteFavoriteSongLink = `action= "/melodydreams/process/createNewFavoriteSong/${song.id}"`
