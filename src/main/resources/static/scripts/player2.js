...
const updateLoadedUI = () => {
    ...
    currentTrackDescription = `${music_list[track_index].description} From ${music_list[track_index].album}`;
...
const repeatTrackMode = () => {
    curr_track.forEach((element) => {
        ...
        loadTrack(music_list[current_index].trackId);
...
/*
		edit_form.forEach(panel => {
			panel.classList.add('hidePanel');
		});
		
		delete_form.forEach(panel => {
			panel.classList.add('hidePanel');
		});
*/		
	}
...
let currentTrackId = null;let writeTextTimeout = null;
...
const updateElements = () => {
    ...
    track_description.forEach((panel, i) => {
        if (isPlaying) {
            panel.classList.remove('hidePanel');
            panel.textContent = `Now Streaming ${currentTrackDescription}`.slice(0, promptIdx);
...
/*
const writeGeneralText = (element, text) => {
    if (!text) {
	...
	if (!curr_track.paused) {
		element.innerText = text.slice(0, countIdx);
		element.style.boxShadow =`1px 1px 3px ${random_hex_color()}`;  
		element.style.borderBottom =`1px solid ${random_hex_color()}`;  
        element.style.transitionProperty = 'all';
        element.style.transitionDuration = `${bpm / 3.3 / bpm}s`;
	    ...
	    setTimeout(() => writeGeneralText(element, text), speed);
	} else if (current_song_index !== track_index) {
		countIdx = 1;
		element.innerText = '';
		element.innerText = main_greeting_message;
    	clearTimeout(writeGeneralText);
...
function displayFileName(inputId, targetId, detailsDivId) {
    ...
    if (input.files.length > 0) {
        ...
        if (fileName.match(/\.(mp3|ogg|wav)$/)) {
            let reader = new FileReader();
            reader.onload = function (e) {
	            let audioSnippet = '<p>Audio Preview:</p><audio controls src="' + URL.createObjectURL(file) + '" type="audio/mpeg"></audio>';
